...
alpha = np.radians(30)  # Rotation angle around X-axis
beta = np.radians(0)   # Rotation angle around Y-axis
gamma = np.radians(0)  # Rotation angle around Z-axis
...
ax.scatter(rotated_point_one[:, 0], rotated_point_one[:, 1], rotated_point_one[:, 2], c='b', marker='o')
ax.scatter(object_pts[:, 0], object_pts[:, 1], object_pts[:, 2], c='r', marker='^', label='Point Cloud 2')
...
# Define the translation amount along the x-axis (0.1 meters to the right)
translation_amount = 0.0

# Define the rotation angle (if rotation is needed)
# For example, rotating around the z-axis by angle theta (in radians)
theta = np.radians(0)  # Example rotation angle of 30 degrees
...
# Translation vector (0.1 meters along the x-axis)
translation_vector = np.array([translation_amount, 0, 0])

# Create the combined transformation matrix (rotation followed by translation)
# Stack the rotation_matrix with the translation_vector to form a (3, 4) matrix
no_change_matrix = np.column_stack((rotation_matrix, translation_vector))


print("ADD: {}".format(add_metric(no_change_matrix, rotation_translation_matrix, object_name)))

print("ADDs: {}".format(cal_adds_numpy(rotation_translation_matrix, no_change_matrix, object_pts)))

rotation_error = re(rotation_translation_matrix[:, :3].T, no_change_matrix[:, :3].T)
translation_error = te(rotation_translation_matrix[:, 3], no_change_matrix[:, 3])
