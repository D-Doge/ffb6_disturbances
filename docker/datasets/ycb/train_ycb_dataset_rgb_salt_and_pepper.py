...
class Dataset_train_rgb_salt_and_pepper():

    def __init__(self, dataset_name, percentage, DEBUG=False):
    ...
    def set_percentage_to_salt_and_pepper(self, image, percentage):
        """
        Sets a certain percentage of pixels in the image to a specified value.

        Parameters:
            image (numpy.ndarray): Input image array (RGB).
            percentage (float): Percentage of pixels to modify.
            value: Value to set the selected pixels.

        Returns:
            numpy.ndarray: Image array with specified percentage of pixels modified.
        """
        ...
        num_pixels_to_modify = int((percentage / 100.0) * num_pixels)

        # Randomly select pixels to modify
        indices_to_modify = np.random.choice(num_pixels, num_pixels_to_modify, replace=False)

        # Flatten the image
        flattened_image = image.reshape(-1, image.shape[-1])

        cut = int(len(indices_to_modify) / 2)
        indices_black = indices_to_modify[:cut]
        # Black
        for i in range(flattened_image.shape[-1]):
            channel = flattened_image[:, i]
            channel[indices_black] = 0

        indices_white = indices_to_modify[cut:]
        # White
        for i in range(flattened_image.shape[-1]):
            channel = flattened_image[:, i]
            channel[indices_white] = 255
    ...
    def get_item(self, item_name):
        ...
        with Image.open(os.path.join(self.root, item_name+'-color.jpg')) as ri:
            ...
            if np.random.rand() <= 0.8:
                rgb = self.set_percentage_to_salt_and_pepper(rgb, int(np.random.rand()*100))
