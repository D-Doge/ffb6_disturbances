...
class Dataset_Brightness():

    def __init__(self, dataset_name, brightness_factor, DEBUG=False):
    ...
    def adjust_brightness(self, image, factor):
        """
        Adjusts the brightness of the image array by scaling the pixel values.

        Parameters:
            image (numpy.ndarray): Input image array.
            factor (float): Brightness adjustment factor. Values > 1 increase brightness, values < 1 decrease brightness.

        Returns:
            numpy.ndarray: Brightened image array.
        """
        # Scale pixel values by the factor
    ...
    def get_item(self, item_name):
        ...
        with Image.open(os.path.join(self.root, item_name+'-color.jpg')) as ri:
            ...
            rgb = np.array(ri)[:, :, :3]
            rgb = ri = self.adjust_brightness(rgb, self.brightness_factor)
