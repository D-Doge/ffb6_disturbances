...
class Dataset_depth_missing_values():

    def __init__(self, dataset_name, missing_percentage, DEBUG=False):
    ...
    def set_percentage_to_one(self, depth_image, percentage):
        """
        Sets a certain percentage of pixels in the depth image to 0.

        Parameters:
            depth_image (numpy.ndarray): Input depth image array.
            percentage (float): Percentage of pixels to set to 0.

        Returns:
            numpy.ndarray: Depth image array with specified percentage of pixels set to 0.
        """
        # Calculate the number of pixels to set to 0
        num_pixels_to_zero = int((percentage / 100) * depth_image.size)
        
        # Randomly select pixels to set to 0
        indices_to_zero = np.random.choice(depth_image.size, num_pixels_to_zero, replace=False)
        
        # Flatten the depth image and set selected pixels to 0
        flattened_depth_image = depth_image.flatten()
        flattened_depth_image[indices_to_zero] = 1
        
        # Reshape the depth image to its original shape
        modified_depth_image = flattened_depth_image.reshape(depth_image.shape)
    ...
    def get_item(self, item_name):
        with Image.open(os.path.join(self.root, item_name+'-depth.png')) as di:
            dpt_um = np.array(di)
            dpt_um = self.set_percentage_to_one(dpt_um, self.missing_percentage)
