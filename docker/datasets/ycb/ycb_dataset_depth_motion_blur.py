...
class Dataset_depth_motion_blur():

    def __init__(self, dataset_name, motion_blur_length, DEBUG=False):
        self.motion_blur_length = motion_blur_length
    ...
    def randmom_linear_motion_blur(self, img, length):
        """
        Apply linear motion blur to a single-channel 16-bit depth image with a random angle.

        Args:
            img (numpy.ndarray): Input image (16-bit, single-channel).
            length (int): Length of the motion blur effect.

        Returns:
            numpy.ndarray: Image with simulated motion blur.
        """
        ...
        # Determine the size of the kernel based on the direction and length
        a = int(max(list(map(abs, (dx, dy)))) * length * 2)
        if a <= 0:
            return img

        # Create the kernel and draw the motion blur line
        kern = np.zeros((a, a), dtype=np.float32)
        cx, cy = a // 2, a // 2
        dx, dy = list(map(int, (dx * length + cx, dy * length + cy)))
        cv2.line(kern, (cx, cy), (dx, dy), 1.0)
        ...
        # Apply the motion blur kernel to the single-channel 16-bit image
        img_blur = cv2.filter2D(img, -1, kern, borderType=cv2.BORDER_REPLICATE)
        
        # Ensure the result stays in the 16-bit range
        img_blur = np.clip(img_blur, 0, 65535).astype(np.uint16)
    ...
    def get_item(self, item_name):
        with Image.open(os.path.join(self.root, item_name+'-depth.png')) as di:
            dpt_um = np.array(di)
            dpt_um = self.randmom_linear_motion_blur(dpt_um, self.motion_blur_length)

        #im = Image.fromarray(dpt_um)
        #im.save("depth_motion_blur.png")
        #print(item_name)
        #exit()
