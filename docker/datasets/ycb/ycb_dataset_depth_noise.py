...
class Dataset_depth_noise():

    def __init__(self, dataset_name, noise_std, DEBUG=False):
    ...
    def add_random_noise(self, depth_image, percentage=100, noise_std=1.0):
        """
        Adds random noise to a certain percentage of pixels in the depth image.

        Parameters:
            depth_image (numpy.ndarray): Input depth image array.
            percentage (float): Percentage of pixels to add noise to.
            noise_std (float): Standard deviation of the Gaussian noise.

        Returns:
            numpy.ndarray: Depth image array with random noise added to specified percentage of pixels.
        """
        num_pixels_to_noise = int((percentage / 100) * depth_image.size)

        # Randomly select pixels to add noise
        indices_to_noise = np.random.choice(depth_image.size, num_pixels_to_noise, replace=False)

        # Generate random noise from a normal distribution, scaled to uint16 range
        noise = np.random.normal(0, noise_std, num_pixels_to_noise).astype(np.int16)

        # Flatten the depth image and add noise to the selected pixels
        flattened_depth_image = depth_image.flatten().astype(np.int32)  # Use int32 to avoid overflow
        flattened_depth_image[indices_to_noise] += noise

        # Clip the values to ensure they remain within the valid uint16 range
        flattened_depth_image = np.clip(flattened_depth_image, 0, np.iinfo(np.uint16).max)

        # Reshape the depth image to its original shape
        modified_depth_image = flattened_depth_image.astype(np.uint16).reshape(depth_image.shape)
    ...
    def get_item(self, item_name):
        with Image.open(os.path.join(self.root, item_name+'-depth.png')) as di:
            dpt_um = np.array(di)
            dpt_um = self.add_random_noise(dpt_um, noise_std=self.noise_std)

            #im = Image.fromarray(dpt_um)
            #im.save("temp.png")
            #print(item_name)
            #exit()
