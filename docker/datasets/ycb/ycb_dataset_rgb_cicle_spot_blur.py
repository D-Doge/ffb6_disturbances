...
class Dataset_rgb_circle_spot_blur():

    def __init__(self, dataset_name, amount_of_spots, min_radius, max_radius, min_blur, max_blur, DEBUG=False):
        self.amount_of_spots = amount_of_spots
    ...
    def blur_random_circle(self, image, min_radius, max_radius, min_blur_amount, max_blur_amount):
        """
        Apply Gaussian blur to a circular region with a random center in the given RGB image.

        Args:
            image (numpy.ndarray): Input RGB image (3-channel).
            min_radius (int): Minimum radius of the circle.
            max_radius (int): Maximum radius of the circle.
            min_blur_amount (int): Minimum blur amount (minimum kernel size).
            max_blur_amount (int): Maximum blur amount (maximum kernel size).

        Returns:
            numpy.ndarray: Modified image with the random circular region blurred.
        """
        ...
        # Generate random center coordinates within the image
        ...
        # Generate random blur amount within the specified range
        blur_amount = random.randint(min_blur_amount, max_blur_amount)
        blur_amount = blur_amount if blur_amount % 2 == 1 else blur_amount + 1  # Ensure odd size
        blur_amount = max(blur_amount, 3)  # Ensure minimum size is 3 (minimum allowed for GaussianBlur)

        # Create a mask with the circular region
        mask = np.zeros((height, width), dtype=np.uint8)
        cv2.circle(mask, center, radius, 255, -1)

        # Apply Gaussian blur to the circular region only
        blurred_region = cv2.GaussianBlur(modified_image, (blur_amount, blur_amount), 0)
        modified_image[mask == 255] = blurred_region[mask == 255]
    ...
    def get_item(self, item_name):
        ...
        if item_name[:8] != 'data_syn' and int(item_name[5:9]) >= 60:
            ...
            K = config.intrinsic_matrix['ycb_K1']
                                                        ...
                                                        # Needs to be color.jpg?
        with Image.open(os.path.join(self.root, item_name+'-color.jpg')) as ri:
            if self.add_noise:
                ri = self.trancolor(ri)
            rgb = np.array(ri)[:, :, :3]
            for i in range(self.amount_of_spots):
                rgb = self.blur_random_circle(rgb, self.min_radius, self.max_radius, self.min_blur, self.max_blur)

        #im = Image.fromarray(rgb)
        #im.save("RGB_Circle_Spot_Blur.jpg")
        #exit()
