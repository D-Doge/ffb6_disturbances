...
class Dataset_rgb_missing_values():
    ...
    def set_percentage_to_value(self, image, percentage, value):
        """
        ...
        # Modify selected pixels for each channel
        for i in range(flattened_image.shape[-1]):
            channel = flattened_image[:, i]
            channel[indices_to_modify] = value[i]
    ...
    def get_item(self, item_name):
        ...
        with Image.open(os.path.join(self.root, item_name+'-color.jpg')) as ri:
            ...
            rgb = np.array(ri)[:, :, :3]
            rgb = self.set_percentage_to_value(rgb, self.percentage, [0, 0, 0])
