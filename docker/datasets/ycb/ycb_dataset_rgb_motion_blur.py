...
class Dataset_rgb_motion_blur():
    ...
    def randmom_linear_motion_blur(self, img, length):
        """
        Apply linear motion blur to an image with a random angle.

        Args:
            img (numpy.ndarray): Input image (3-channel RGB).
    ...
    def get_item(self, item_name):
        ...
        with Image.open(os.path.join(self.root, item_name+'-color.jpg')) as ri:
            ...
            rgb = np.array(ri)[:, :, :3]
            rgb = self.randmom_linear_motion_blur(rgb, self.motion_blur_length)

        #im = Image.fromarray(rgb)
        #im.save("RGB_Motion_Blur.jpg")
        #exit()
