...
class Dataset_rgb_noise():
    ...
    def add_random_noise(self, rgb_image, percentage=100, noise_std=1.0):
        """
        ...
        num_pixels_to_noise = int((percentage / 100) * rgb_image.shape[0] * rgb_image.shape[1])

        # Randomly select pixels to add noise to
        indices_to_noise = np.random.choice(rgb_image.shape[0] * rgb_image.shape[1], num_pixels_to_noise, replace=False)

        # Generate random noise for each channel from a normal distribution
        noise = np.random.normal(0, noise_std, (num_pixels_to_noise, 3)).astype(np.int16)

        # Flatten the RGB image for easier indexing
        flattened_rgb_image = rgb_image.reshape(-1, 3).astype(np.int32)  # Use int32 to avoid overflow

        # Add noise to the selected pixels
        flattened_rgb_image[indices_to_noise] += noise

        # Clip the values to ensure they remain within the valid uint8 range
        flattened_rgb_image = np.clip(flattened_rgb_image, 0, 255)

        # Reshape the image back to its original shape
        modified_rgb_image = flattened_rgb_image.astype(np.uint8).reshape(rgb_image.shape)
    ...
    def get_item(self, item_name):
        ...
        with Image.open(os.path.join(self.root, item_name+'-color.jpg')) as ri:
            ...
            rgb = np.array(ri)[:, :, :3]
            rgb = self.add_random_noise(rgb, noise_std=self.noise_std)
