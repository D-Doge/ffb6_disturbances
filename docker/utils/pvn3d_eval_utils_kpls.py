...
def cal_frame_poses(
    ...
    if device == 'cuda':
        ...
        if use_ctr:
            ...
            cls_kps = torch.zeros(n_cls, n_kps, 3)
    ...
    if use_ctr_clus_flter:
        ...
        try:
            if device == 'cuda':
                ...
                ctrs = torch.from_numpy(np.array(ctrs).astype(np.float32))
            n_ctrs, _ = ctrs.size()
            ...
            if device == 'cuda':
                ...
                msk_closest_ctr = torch.LongTensor(pred_cls_ids)[min_idx]
            new_msk = mask.clone()
    ...
    for icls, cls_id in enumerate(pred_cls_ids):
        ...
        # visualize the separate keyframes, used to be if debug
        if False:
            ...
            cv2.imwrite("/workspace/dd6d/datasets/ycb/YCB_Video_Dataset/ffb6d_visuals/img_kps_" +str(cls_id)+".jpg", show_kp_img)
        ...
        #Saves the mesh and kps xyz Files
        if False:
            with open("/workspace/dd6d/datasets/ycb/YCB_Video_Dataset/ffb6d_visuals/mesh_kps_" +str(cls_id)+".xyz", 'w') as file:
                for line in mesh_kps: 
                    file.write(str(line)[1:-1] + "\n")
            with open("/workspace/dd6d/datasets/ycb/YCB_Video_Dataset/ffb6d_visuals/pred_kpc_" +str(cls_id)+".xyz", 'w') as file:
                for line in pred_kpc:
                    file.write(str(line)[1:-1] + "\n")
        pred_RT = best_fit_transform(mesh_kps, pred_kpc)
        #print("String1: " + str(pred_RT.shape))
        #print(pred_RT)
        #pred_RT = tree_IPC(mesh_kps, pred_kpc)
        #print("String2: " + str(pred_RT.shape))
        #print(pred_RT)
        pred_kps_lst.append(pred_kpc)
        pred_pose_lst.append(pred_RT)

    #print("Keypoint_Clouds length: " + str(len(pred_kps_lst)))
    #print("Rotations length: " + str(len(pred_pose_lst)))
    return (pred_cls_ids, pred_pose_lst, pred_kps_lst)

# def tree_IPC(A, B):
#     tree_icp = TreeICP(0.99, 10, tree_depth=1, tree_breadth=5)
#     #tree_icp = ICP(2)
#     #tree_icp = tree_icp.fit(A, B, initial_transform = np.identity(4))
#     tree_icp = tree_icp.fit(A, B)
#     ret = tree_icp.predict(A)
#     print(ret)
#     ret = ret[:3,:]
#     return ret
